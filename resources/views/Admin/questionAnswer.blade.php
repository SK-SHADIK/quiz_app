    ...
    <section class="crud-section">
                ...
                <thead>
                    <tr class="crud-table-headers">
                        <th class="crud-table-header">
                            <a
                                href="{{ route('sort.id.question.answers', ['sort_column' => 'id', 'sort_order' => 'asc']) }}">Id
                        ...
                        <th class="crud-table-header">
                            <a
                                href="{{ route('sort.marks.question.answers', ['sort_column' => 'marks', 'sort_order' => 'asc']) }}">Marks
                                <img class="sorting-icon" src="{{ asset('img/sorting.png') }}" alt=""></a>
                        </th>
                        ...
                        @foreach ($questionAnswers as $questionAnswer)
                            <tr class="crud-table-body">
                                ...
                                <td class="crud-table-row">
                                    <form method="POST"
                                        action="{{ route('active.deactive.question.answer', ['id' => $questionAnswer['id']]) }}">
                                        @csrf
                                        @method('POST')
                                        <label class="switch">
                                            <input type="checkbox" class="toggle-switch" name="is_active" value="1"
                                                @if ($questionAnswer['is_active']) checked @endif
                                ...
                                <td class="crud-table-row" colspan="3"><a
                                        href="/quiz-app/admin/view-question-answer/{{ $questionAnswer['id'] }}"
                                        class="crud-view-button">View</a>
                                    <a href="/quiz-app/admin/edit-question-answer/{{ $questionAnswer['id'] }}"
                                        class="crud-edit-button">Edit</a>
                                    <a href="#" class="crud-delete-button"
                                        data-question-id="{{ $questionAnswer['id'] }}">Delete</a>
                        @endforeach
                    @else
                        <tr>
        ...
        <tfoot>
            <div class="custom-pagination">
                ...
                <span class="pagination-info">Page {{ $questionAnswers->currentPage() }} of
                    {{ $questionAnswers->lastPage() }}</span>
